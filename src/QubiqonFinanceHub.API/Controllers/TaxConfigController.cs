...
public class TaxConfigController(ITaxConfigService svc) : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CreateTaxConfigRequest dto) => Ok(await svc.CreateAsync(dto));
    ...
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateTaxConfigRequest dto) => Ok(await svc.UpdateAsync(id, dto));
